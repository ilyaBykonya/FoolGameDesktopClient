...
void GameTableWidget::serverSlotPlayerTossedCard(qint16 playerID, Card::Suit cardSuit, Card::Dignity cardDignity)
{
    if(playerID == m_thisPlayerID)
    {
        for(auto it = m_thisPlayerActionsList.begin(); it != m_thisPlayerActionsList.end(); ++it)
        {
            if((*it)->type() == PlayerActionType::Toss)
            {
                if(((*it)->suit() == cardSuit) && ((*it)->dignity() == cardDignity))
                {
                    delete (*it);
                    m_thisPlayerActionsList.erase(it);
                    return;
                }
            }
        }
    }
    Card* card = findDeckForID(playerID)->takeCard(cardSuit, cardDignity, this);
...
void GameTableWidget::serverSlotPlayerBeatCard(qint16 playerID, qint16 battlePairID, Card::Suit cardSuit, Card::Dignity cardDignity)
{
    if(playerID == m_thisPlayerID)
    {
        for(auto it = m_thisPlayerActionsList.begin(); it != m_thisPlayerActionsList.end(); ++it)
        {
            if((*it)->type() == PlayerActionType::Beat)
            {
                if(((*it)->suit() == cardSuit) && ((*it)->dignity() == cardDignity))
                {
                    delete (*it);
...
void GameTableWidget::serverSlotPlayerTransferable(qint16 playerID, Card::Suit cardSuit, Card::Dignity cardDignity)
{
    if(playerID == m_thisPlayerID)
    {
        for(auto it = m_thisPlayerActionsList.begin(); it != m_thisPlayerActionsList.end(); ++it)
        {
            if((*it)->type() == PlayerActionType::Trancferable)
            {
                if(((*it)->suit() == cardSuit) && ((*it)->dignity() == cardDignity))
                {
                    delete (*it);
    ...
    Card* card = this->findDeckForID(playerID)->takeCard(cardSuit, cardDignity, this);
    if(card)
        m_battleAreaWidget->tossCard(card);
}
...
//Неудачные действия.
void GameTableWidget::serverSlotUnsuccesfullyAction(PlayerActionType actionType, Card::Suit cardSuit, Card::Dignity cardDignity, qint16 pairID)
{

    for(auto it = m_thisPlayerActionsList.begin(); it != m_thisPlayerActionsList.end(); ++it)
    {
        if((*it)->type() == actionType)
        {
            if(((*it)->suit() == cardSuit) && ((*it)->dignity() == cardDignity))
            {
                    if(actionType == PlayerActionType::Beat) {
                        Card* card = m_battleAreaWidget->cancelBeat(cardSuit, cardDignity, pairID);
                        if(card)
                        {
                            QObject::connect(card, &Card::dropThisElement, this, &GameTableWidget::playerChangeThisCard, Qt::ConnectionType::UniqueConnection);
                            QObject::connect(card, &Card::dragThisElement, this, &GameTableWidget::startCardMove, Qt::ConnectionType::UniqueConnection);
                        ...
                        Card* card = m_battleAreaWidget->cancelToss(cardSuit, cardDignity, pairID);
                        if(card)
                        {
                            QObject::connect(card, &Card::dropThisElement, this, &GameTableWidget::playerChangeThisCard, Qt::ConnectionType::UniqueConnection);
                            QObject::connect(card, &Card::dragThisElement, this, &GameTableWidget::startCardMove, Qt::ConnectionType::UniqueConnection);
                ...
                m_thisPlayerActionsList.pop_front();
                return;
            }
        }
    }
            ...
            if(pair != nullptr)
            {
                if(pair->state() == BattlePairWidget::PairState::NotBeaten)
                {
                    if(m_battleAreaWidget->beatCard(card, pair->id()))
                    {
                        ...
                        m_thisPlayerActionsList.push_back(new PlayerAction(PlayerActionType::Beat, card->suit(), card->dignity(), pair->id()));
                ...
                {
                    if(m_battleAreaWidget->trancferableCard(card))
                    {
                        QObject::disconnect(card, &Card::dragThisElement, this, &GameTableWidget::startCardMove);
                        QObject::disconnect(card, &Card::movingThisElement, this, &GameTableWidget::playerMoveCard);
                        QObject::disconnect(card, &Card::dropThisElement, this, &GameTableWidget::playerChangeThisCard);
                        ...
                        m_battleAreaWidget->playerLoweredCard(card);
                        m_thisPlayerActionsList.push_back(new PlayerAction(PlayerActionType::Trancferable, card->suit(), card->dignity(), pair->id()));
        ...
        else
        {
            ...
            if(pair != nullptr)
            {
                if(m_battleAreaWidget->tossCard(card))
                {
                    ...
                    QObject::disconnect(card, &Card::dropThisElement, this, &GameTableWidget::playerChangeThisCard);
                    m_battleAreaWidget->playerLoweredCard(card);
                    m_thisPlayerActionsList.push_back(new PlayerAction(PlayerActionType::Toss, card->suit(), card->dignity(), pair->id()));
...
{
    qDebug() << "TABLE: playerMoveCard";
    QRect rect(m_battleAreaWidget->pos(), m_battleAreaWidget->size());
    if(rect.contains(p))
    {
        if(m_thisPlayer->queue() == QueueMove::Defender)
        {
            qDebug() << "   playerMoveCard (TRUE)";
