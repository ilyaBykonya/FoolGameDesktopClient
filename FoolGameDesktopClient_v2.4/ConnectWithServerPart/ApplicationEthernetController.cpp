...
void ApplicationEthernetController::applicationSlotPlayerRegistration(const QString& name, const QString& password)
{
    ...
    outStream << qint16(0)
              ...
              << EthernetSignals::ClientSignal::ApplicationSignal::PlayerRegistration
              << name
              << password;
    outStream.device()->seek(0);
    outStream << qint16(dataBlock.size() - sizeof(qint16));
...
void ApplicationEthernetController::serverSlotReadFromServInstToAppInst(QDataStream& dataBlock)
{
    ...
    switch (typeMessage)
    {
        ...
        case EthernetSignals::ServerSignal::InstanceSignal::SetNewMainPlayersPair           :
        {
            qint16 newAttackerID;
            qint16 newDefenderID;
            dataBlock >> newAttackerID >> newDefenderID;
            qDebug() << QString("READ SET_NEW_MAIN_PAIR [%1::%2]").arg(newAttackerID).arg(newDefenderID);
            emit this->signalInstanceSetNewMainPair(newAttackerID, newDefenderID);
        ...
        case EthernetSignals::ServerSignal::InstanceSignal::YourActionIsUnsuccesfully       :
        {
            qDebug() << QString("READ UNSUCCESFULLY_MESSAGE");
            emit this->signalInstanceYourActionIsUnsuccesfully();
            break;
        }
