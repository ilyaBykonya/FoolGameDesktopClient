...
void StartDeckWidget::paintEvent(QPaintEvent *)
{
    ...
    if(m_countOfCards > 0)
    {
        ...
        {
            painter.drawPixmap(QRect((-this->width() / 4 - i / 5),
    ...
    {
        QPixmap trumpPixmap(QString("D:/FoolGame/FoolGameDesktopClient/pictures/cards_suit/%1.png").arg(m_trump));
        painter.drawPixmap(QRect(5, ((this->height() / 2) - (this->width() / 2)), this->width() - 10, (this->height() / 2)), trumpPixmap);
    }
}
