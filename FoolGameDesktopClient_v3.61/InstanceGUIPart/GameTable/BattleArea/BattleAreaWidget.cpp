...
BattlePairWidget* BattleAreaWidget::addNewPair()
{
    ...
    BattlePairWidget* newPair = new BattlePairWidget(m_thisRealPos, this);
...
BattlePairWidget* BattleAreaWidget::checkRightTossingCard(Card* card)
{
...
BattlePairWidget* BattleAreaWidget::checkRightTrancferableCard(Card* card)
{
    if((m_pairsList.size() < 4) && (m_pairsList.size() > 0))
    {
        return m_pairsList.last();
    }
    return nullptr;
