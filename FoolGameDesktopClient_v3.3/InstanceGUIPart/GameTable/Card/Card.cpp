...
void Card::doAnimationOnPoint(QPoint p)
{
    ...
    qint16 lengthOfPath = sqrt(pow(diff.width(), 2) + pow(diff.height(), 2));
    QPropertyAnimation* animation = new QPropertyAnimation(this, "geometry", this);
    animation->setStartValue(QRect(this->pos(), this->size()));
    animation->setEndValue(QRect(p, m_pictureCard.size()));
    animation->setDuration(lengthOfPath / 3);

    QObject::connect(animation, &QPropertyAnimation::finished, this, &Card::successDrop);
